...
- Env di `.env`: `ADMIN_PASSWORD` (default `ipcheck`), `TRUST_PROXY=true`, `TRUSTED_PROXIES=` (kosong → semua proxy dipercaya), `APP_TIMEZONE=Asia/Jakarta`.
...
- Data dirotasi otomatis: `visitor_data.jsonl` maks 10 MB, sisakan 2000 baris terakhir (`LOG_MAX_BYTES`, `LOG_KEEP_LINES` di config.php).
- `parse_data()` di admin.php: urut dari terbaru, pakai `tracked_at` kalau ada fallback ke `time`.
- `load_data()` key = `visit_id`; `ip_visit_count()` memakai `substr_count` pada file (harus string `"ip":"..."`).
